...
class NovelAgent_Client(BaseLLMModel):
    def __init__(self, model_name, api_key, user_name="", base_url="http://209.97.149.43:2344") -> None:
        super().__init__(model_name=model_name, user=user_name, config={"api_key": api_key})
        self.base_url = base_url
        self.session_id = None
        self.history = []
        self.system_prompt = """You are a novel agent that helps users analyze and understand paper works. 
        You can provide insights on papers"""

    def _get_novel_style_input(self):
        messages = []
    ...
    def get_answer_at_once(self):
        try:
            messages = self._get_novel_style_input()
            ...
            logging.error(f"Error in NovelAgent: {str(e)}")
